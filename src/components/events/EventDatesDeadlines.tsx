...
/**
 * Redesigned EventStagesTimeline component strictly following the Studio.Dei "Timeline Protocol" design.
 * It maintains the original logic for stage status and date formatting.
 */
export function EventStagesTimeline({ stages }: EventStagesTimelineProps) {
  if (!stages || stages.length === 0) return null;

  return (
    <div className="w-full max-w-[800px] mx-auto font-sans antialiased bg-black text-white">
      {/* Technical Header */}
      <div className="flex items-center gap-[15px] mb-[60px]">
        <div className="w-[2px] height-[24px] bg-[#ff8c00]" />
        <h2 className="font-serif text-[2.2rem] font-normal tracking-[-0.5px]">
          Stages & Timeline
        </h2>
      </div>
        ...
        {stages.map((stage, index) => {
          ...
          return (
            <div key={stage.id} className="relative mb-[30px] last:mb-0">
              {/* Industrial Markers: Square & Bordered */}
              <div 
                className={cn(
                  "absolute left-[-35px] top-[4px] w-[21px] h-[21px] bg-black border z-[2] flex items-center justify-center",
                  isCompleted ? "border-[#00ff88] text-[#00ff88]" : 
                  isActive ? "border-white" : "border-[#111]"
                )}
              >
              ...
              <div 
                className={cn(
                  "bg-[#050505] border border-[#1a1a1a] p-[30px] flex flex-col md:flex-row justify-between items-start",
                  isCompleted && "opacity-50"
                )}
              >
                <div className="step-info flex-1">
                  <h4 className="text-[0.65rem] text-[#555555] uppercase tracking-[3px] mb-[12px] font-semibold">
                    Protocol {String(index + 1).padStart(2, '0')}
                  </h4>
                  <h3 className="font-serif text-[1.4rem] font-normal mb-[10px]">
                    {stage.name}
                  </h3>
                  ...
                  {/* LED STATUS BLOCK: Active stage only */}
                  {isActive && (
                    <div className="mt-[25px] inline-flex items-center gap-[12px] px-4 py-2 border border-[#1a1a1a] bg-white/2">
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .animate-pulse {
          animation: led-glow 2.5s infinite ease-in-out;
        }
      `}} />
    </div>
  );
