...
export default function PracticeSolver() {
  ...
  const handleRun = async () => {
    ...
    // Switch to console view on mobile when running
  ...
  const ProblemPanel = () => (
    <Tabs value={descriptionTab} onValueChange={(v) => setDescriptionTab(v as any)} className="flex flex-col h-full">
      <div className="h-10 border-b border-white/5 flex items-center px-2 bg-[#0a0a0c] shrink-0">
        <TabsList className="h-full bg-transparent p-0 gap-1 w-full justify-start overflow-x-auto no-scrollbar">
          {[
            ...
            <TabsTrigger 
              key={tab.value} 
              value={tab.value} 
              className="relative h-8 rounded-md px-3 text-[11px] font-bold uppercase tracking-wider text-zinc-500 data-[state=active]:text-white data-[state=active]:bg-white/5 transition-all hover:text-zinc-300 whitespace-nowrap"
            >
          ...
          <ScrollArea className="flex-1">
            <div className="p-4 md:p-6 pb-24 max-w-3xl mx-auto">
              <div className="mb-6 pb-6 border-b border-white/5">
                <h2 className="text-xl md:text-2xl font-bold text-white mb-3">{problem.title}</h2>
            ...
            <div className="p-6">
              {problem.editorial ? (
                ...
                <div className="flex flex-col items-center justify-center h-[400px] text-zinc-600">
                  ...
                  <p className="text-sm font-bold uppercase tracking-widest">Classified</p>
                </div>
              )}
            </div>
          </ScrollArea>
      ...
      <AnimatePresence>
        {executionResult?.passed && !isMobile && (
          <motion.div 
  ...
  const ConsolePanel = () => (
    <div className="bg-[#08080a] flex flex-col h-full">
      <Tabs value={consoleTab} onValueChange={(v) => setConsoleTab(v as any)} className="w-full h-full flex flex-col">
        <div className="h-9 border-b border-white/5 flex items-center px-2 bg-[#0c0c0e] shrink-0 justify-between">
          <TabsList className="h-full bg-transparent p-0 gap-4 overflow-x-auto no-scrollbar justify-start">
            <TabsTrigger value="testcase" className="h-full rounded-none border-b-2 border-transparent data-[state=active]:border-white data-[state=active]:text-white text-xs font-medium text-zinc-500 flex items-center gap-2 px-2 hover:text-zinc-300 transition-colors whitespace-nowrap">
              <CheckCircle2 className="w-3.5 h-3.5" /> Testcase
            </TabsTrigger>
            <TabsTrigger value="custom" className="h-full rounded-none border-b-2 border-transparent data-[state=active]:border-purple-500 data-[state=active]:text-purple-400 text-xs font-medium text-zinc-500 flex items-center gap-2 px-2 hover:text-zinc-300 transition-colors whitespace-nowrap">
              <Beaker className="w-3.5 h-3.5" /> Custom
            </TabsTrigger>
            <TabsTrigger value="result" className="h-full rounded-none border-b-2 border-transparent data-[state=active]:border-blue-500 data-[state=active]:text-blue-400 text-xs font-medium text-zinc-500 flex items-center gap-2 px-2 hover:text-zinc-300 transition-colors whitespace-nowrap">
            ...
            <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-white/10">
              {testCases.filter(t => t.is_public).map((_, i) => (
                <button 
                  ...
                  className={cn(
                    "px-4 py-1.5 rounded text-xs font-bold uppercase tracking-wider transition-all border shrink-0", 
                    activeTestCaseId === i 
  ...
  return (
    <div className="h-screen flex flex-col bg-[#050505] text-white overflow-hidden font-inter selection:bg-primary/30 relative">
      ...
      {/* --- HEADER --- */}
      <header className="h-14 md:h-16 shrink-0 z-50 px-3 md:px-4 flex items-center justify-between border-b border-white/5 bg-[#050505]/80 backdrop-blur-md">
        <div className="flex items-center gap-2 md:gap-4">
          <Button variant="ghost" size="icon" onClick={() => navigate('/practice-arena')} className="w-8 h-8 md:w-10 md:h-10 rounded-full border border-white/5 bg-white/5 hover:bg-white/10 hover:border-white/10 text-zinc-400 hover:text-white transition-all group">
          ...
          <div className="flex flex-col max-w-[150px] md:max-w-none">
             <div className="flex items-center gap-2 md:gap-3">
               <h1 className="font-bold text-xs md:text-base text-white tracking-tight truncate">{problem.title}</h1>
               <div className={cn("hidden md:block px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider border", DifficultyColor)}>
                 {problem.difficulty}
               </div>
             </div>
             {!isMobile && (
        ...
        {/* Central Timer (Desktop) */}
        {!isMobile && (
          <div className="flex items-center gap-3 px-4 py-1.5 bg-[#0a0a0c] rounded-full border border-white/10 shadow-inner">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          <Select value={activeLanguage} onValueChange={(v) => setActiveLanguage(v as Language)}>
            <SelectTrigger className="h-8 md:h-9 w-[110px] md:w-[140px] bg-[#0a0a0c] border-white/10 text-[10px] md:text-xs font-bold text-zinc-300 focus:ring-primary/20 hover:border-white/20 transition-colors">
              <SelectValue />
            </SelectTrigger>
            ...
            <>
             ...
             <div className="flex items-center gap-1">
                <Button 
                  ...
                  onClick={handleRun}
                  disabled={isRunning || isSubmitting || isJudging}
                  className="h-8 w-8 border-primary/20 text-primary bg-primary/5"
                >
        ...
        {isMobile ? (
          ...
          <>
            <div className="flex-1 relative overflow-hidden">
               ...
               {mobileTab === 'console' && <ConsolePanel />}
            </div>
            
            {/* Bottom Nav Bar */}
            <div className="h-14 bg-[#080808] border-t border-white/10 flex items-center justify-around px-2 shrink-0 z-30">
               <button 
                  onClick={() => setMobileTab('problem')} 
                  className={cn("flex flex-col items-center gap-1 p-2 rounded-lg transition-colors", mobileTab === 'problem' ? "text-white" : "text-zinc-600")}
               >
               ...
               <button 
                  ...
                  className={cn("flex flex-col items-center gap-1 p-2 rounded-lg transition-colors", mobileTab === 'editor' ? "text-white" : "text-zinc-600")}
               >
               ...
               <button 
                  ...
                  className={cn("flex flex-col items-center gap-1 p-2 rounded-lg transition-colors", mobileTab === 'console' ? "text-white" : "text-zinc-600")}
               >
